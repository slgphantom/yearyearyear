...
public class MessageHandle{
	...
	public static void findMon(PokemonGo go, double x, double y){
		...
		System.out.println("ÔwÕ˘◊¯òÀµÿ¸c " + "[" + x + "£¨" + y + "]");
		String str = "";
		try {
			for(int i=0; i < go.getMap().getCatchablePokemon().size(); i++){
				...
				if(encResult.wasSuccessful()){
					System.out.println("’“µΩæ´Ï` " + go.getMap().getCatchablePokemon().get(i).getPokemonId() );
					str += "◊¯òÀ : ";
					...
					str += "\n";
					str += "ººƒ‹1 : " + encResult.getPokemonData().getMove1();
					str += "\n";
					str += "ººƒ‹2 : " + encResult.getPokemonData().getMove2();
					str += "\n\n";
				}
			}
			message = new SendMessage()
